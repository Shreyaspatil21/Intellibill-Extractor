...
class MainExtractor:
    ...
    def extract_line_items(self, word_map=None):
        ...
        global_metrics = [self._get_metrics(l) for l in self.lines]
        # 2. Extract column boundaries from the rules
        # RuleGenerator uses 'Rule type', 'Field Name', 'Column Start', 'Column End'
        current_cols = []
        global_table_start_idx = 0
        global_footer_start_idx = len(global_metrics)
        
        if self.rules:
            for r in self.rules:
                if r.get('Section') == 'COLUMNS':
                    lbl = r.get('Field Name', 'Col').replace("Col: ", "")
                    if not any(c['label'] == lbl for c in current_cols):
                        current_cols.append({
                            'label': lbl,
                ...
                if r.get('Section') == 'STRUCTURE' and r.get('Field Name') == 'Line Item Start':
                    global_table_start_idx = int(r.get('Sample Text', 0)) - 1
                if r.get('Section') == 'STRUCTURE' and r.get('Field Name') == 'Footer Start':
                    global_footer_start_idx = int(r.get('Sample Text', 0)) - 1
        
        # --- Rule-Based Structural Anchoring ---
        if self.rules:
            for r in self.rules:
                if r.get('Section') == 'COLUMNS':
                    lbl = r.get('Field Name', 'Col')
                    if not any(c['label'] == lbl for c in current_cols):
                        current_cols.append({
                            'label': lbl,
                            'start': int(r.get('Column Start', 0)),
        ...
        if not current_cols:
            ...
            for search_idx in range(global_table_start_idx - 1, max(-1, global_table_start_idx - 5), -1):
            ...
            for epoch in range(1, 51):
                ...
                for i in range(max(0, global_table_start_idx), min(len(global_metrics), global_footer_start_idx)):
                    m = global_metrics[i]
                    if not m['is_sep']:
                        for t in m['tokens']:
                ...
                for i in range(max(0, global_table_start_idx), min(len(global_metrics), global_footer_start_idx)):
                    ...
                    for t in global_metrics[i]['tokens']:
        ...
        # Now process the table page-by-page starting from the actual data line until the footer starts
        for page_idx, page_lines in enumerate(raw_pages):
            is_first_page = (page_idx == 0)
            is_last_page = (page_idx == len(raw_pages) - 1)
            
            metrics = [self._get_metrics(l) for l in page_lines]
            page_table_start = global_table_start_idx if is_first_page else 0
            page_footer_start = (global_footer_start_idx - line_offset) if is_last_page else len(page_lines)
            
            # Clamp limits to avoid index errors
            page_footer_start = max(0, min(page_footer_start, len(page_lines)))
            page_table_start = max(0, min(page_table_start, page_footer_start))

            last_item = None
            for i in range(page_table_start, page_footer_start):
                m = metrics[i]
                ...
                if header_hits >= 3 and i > page_table_start + 2:
                    continue
                ...
                if is_header_repeat and i > page_table_start + 3: continue
